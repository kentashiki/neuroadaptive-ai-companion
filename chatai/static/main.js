...
const STYLE_BY_STATE = {
  focused: "丁寧で簡潔、構造的",
  distracted: "親しみやすく、会話的",
};

const DISPLAY_LABEL_BY_STATE = {
  focused: "集中",
  distracted: "散漫",
};
...
async function fetchJson(url, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(payload.message || payload.error || "リクエストに失敗しました");
  }
  return payload;
}
...
function renderState(state) {
  ...
  if (state.estimation?.phase === "calibrating") {
    const elapsed = Number(state.estimation.calibration_elapsed || 0);
    const total = Number(state.estimation.calibration_total || 0);
    attentionValue.textContent = total > 0
      ? `キャリブレーション中 ${Math.floor(elapsed)}/${Math.floor(total)}秒`
      : "キャリブレーション中";
  } else {
    attentionValue.textContent = state.attention.toFixed(2);
  }
  stateLabel.textContent = DISPLAY_LABEL_BY_STATE[state.label] || state.label.replace("_", " ");
...
function buildAssistantBubble(message, meta, body) {
  ...
  avatar.alt = message.state_label === "focused" ? "集中時のAIアイコン" : "散漫時のAIアイコン";
...
function createMessageBubble(message) {
  ...
  meta.textContent = `${message.role === "assistant" ? "AIコンパニオン" : "あなた"} | ${formatTime(message.timestamp)}`;
...
async function refreshMeta() {
  ...
  engineDescription.textContent = payload.llm_enabled
    ...
    : "ルールベース応答（LLMを使うには.envにOPENAI_API_KEYを設定）";
}
...
async function sendMessage(event) {
  ...
  try {
    ...
    if (!response.ok || !response.body) {
      throw new Error("ストリーミング応答に失敗しました");
    }
    ...
    pendingBubble.classList.remove("pending");
    pendingContent.textContent = `エラー: ${error.message}`;
  } finally {
