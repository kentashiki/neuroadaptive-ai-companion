...
from chatai.agent.companion import AICompanionAgent
from chatai.eeg.lsl_focus_receiver import LSLFocusReceiver
from chatai.models.chat import ChatMessage
...
class CompanionSession:
    def __init__(self) -> None:
        self._lock = Lock()
        self.state_source = LSLFocusReceiver()
        self.agent = AICompanionAgent()
        self.chat_history: list[ChatMessage] = []
        self._append_message(
            "assistant",
            "こんにちは。私はNeuroadaptive AI Companionです。現在の認知状態に合わせて、見た目と応答の口調を調整します。",
    ...
    def reset(self) -> None:
        with self._lock:
            ...
            self._append_message(
                "assistant",
                "セッションをリセットしました。準備ができたら、新しい会話を始めてください。",
                state_label=self.state_source.get_state().label,
            )
...
def create_app() -> Flask:
    ...
    @app.get("/api/state")
    def api_state():
        payload = asdict(session.get_state())
        if hasattr(session.state_source, "get_status"):
            payload["estimation"] = session.state_source.get_status()
